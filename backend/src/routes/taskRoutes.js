...
// @desc    Obter tarefas criadas por um professor específico
// @access  Private (professor ou admin da mesma escola)
router.get('/teacher/:teacherId', auth, [
  param('teacherId').isMongoId().withMessage('ID do professor inválido')
], validateRequest, async (req, res) => {
  try {
    const { teacherId } = req.params;
    // Verificar permissões
    if (req.user.role === 'teacher' && req.user._id.toString() !== teacherId) {
      return res.status(403).json({ status: 'error', message: 'Permissão negada: só pode acessar suas próprias tarefas' });
    }

    if (req.user.role === 'school_admin') {
      const teacher = await User.findById(teacherId);
      if (!teacher || teacher.school_id.toString() !== req.user.school_id.toString() || teacher.role !== 'teacher') {
        return res.status(404).json({ status: 'error', message: 'Professor não encontrado ou não pertence à sua escola' });
      }
    }
    ...
    const tasks = await Task.find({
      ...
      school_id: req.user.school_id,
      status: { $ne: 'archived' } // Não mostrar tarefas arquivadas
    })
    .populate('created_by', 'name email')
    .populate('assigned_to.user', 'name email')
    .sort({ createdAt: -1 });
